...
import { PlusCircle, ShieldCheck, ArrowRight, Loader2 } from "lucide-react"
...
export default function DepositPage() {
  ...
  const handleRequestOTP = async (e: React.FormEvent) => {
    ...
    setProcessing(true)
    try {
      const res = await api.post("/transactions/operation-request?op_type=deposit", {
  ...
  if (loading) return <div className="p-12 text-center text-gray-500">Loading Deposit...</div>
  ...
  return (
    <div className="max-w-2xl mx-auto py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-2">Securely add money to your account.</p>
      </div>
        ...
        <Card className="bg-gray-50 border-gray-200">
            <CardContent className="p-6">
                <div className="flex justify-between items-center text-sm font-medium text-gray-500 uppercase tracking-wider mb-4">
                    ...
                    <span className="text-xs text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full ring-1 ring-emerald-200">Verified</span>
                </div>
                <div className="space-y-3">
                    ...
                    <div className="flex justify-between">
                        ...
                        <span className="font-bold text-emerald-600">₹{balance.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                    </div>
                </div>
            </CardContent>
        </Card>
            ...
            <CardContent className="p-8">
              <form onSubmit={handleRequestOTP} className="space-y-6">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      min="1"
                      step="0.01"
                      className="w-full pl-10 pr-4 py-4 bg-gray-50 border-2 border-gray-100 rounded-xl text-2xl font-bold text-gray-900 focus:ring-primary focus:border-primary transition-all outline-none" 
                      placeholder="0.00"
                      value={amount}
                ...
                <button 
                  disabled={processing}
                  type="submit" 
                  className="w-full bg-primary text-white py-4 rounded-xl font-bold text-lg hover:bg-primary/90 transition shadow-lg shadow-primary/20 flex items-center justify-center gap-2 group"
                >
          ...
          <Card className="border-emerald-200 shadow-xl shadow-emerald-50 bg-emerald-50/10">
            <CardContent className="p-8">
                <div className="text-center mb-8">
                    ...
                    <p className="text-sm text-gray-600 mt-2">We sent a 6-digit code to your registered email.</p>
                </div>
              <form onSubmit={handleVerifyOTP} className="space-y-6">
                ...
                >
                  {processing ? <Loader2 className="w-6 h-6 animate-spin" /> : "Verify & Deposit"}
                </button>
